...
export default function Tickets(){
  ...
  const [statusFilter, setStatusFilter] = useState("All");
    ...
    const handleStatusChange = async (ticketId, nextStatus) => {
      ...
      try {
        const result = await employeeService.updateTicketStatus(ticketId, nextStatus);
        if (result?.success && result?.ticket) {
          setTickets((prev) =>
            prev.map((t) => (t._id === ticketId ? { ...t, status: result.ticket.status } : t))
          );
        }
      } catch (err) {
                ...
                <div className="px-6 py-7 border-b border-slate-100 bg-gradient-to-r from-blue-600 via-blue-500 to-blue-400 text-white relative overflow-hidden">
                  ...
                  <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                      ...
                      <h1 className="text-3xl sm:text-4xl font-black">Support Tickets</h1>
                      <p className="text-base text-blue-100">Track, prioritize, and resolve employee issues</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-blue-100">Total Tickets</p>
                      <p className="text-4xl sm:text-5xl font-black">{tickets.length}</p>
                    </div>
                  </div>
                  ...
                  <div className="relative mt-6 grid grid-cols-2 sm:grid-cols-5 gap-3">
                    {[
                      ...
                      >
                        <p className="text-xs text-blue-100 uppercase tracking-wider">{stat.label}</p>
                ...
                <div className="px-6 py-5 border-b border-slate-100 bg-white">
                  <div className="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
                    <div className="flex items-center gap-2 flex-wrap">
                      {["All", "Open", "In Progress", "Resolved", "Closed", "Reopened"].map((status) => (
                        <button
                          key={status}
                  ...
                  {isLoading ? (
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
                      {filteredTickets.map((ticket) => (
                        <div key={ticket._id} className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm hover:shadow-lg transition-shadow relative overflow-hidden">
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="text-base font-bold text-slate-900">{ticket.subject}</p>
                                <p className="text-sm text-slate-500 mt-1">
                                  {ticket.employee?.firstName} {ticket.employee?.lastName} • {ticket.category}
                                </p>
                              </div>
                            </div>
                            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${getStatusStyles(ticket.status)}`}>
                              {ticket.status}
                            </span>
                          </div>
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-4">
                            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${getPriorityStyles(ticket.priority)}`}>
                              {ticket.priority}
                            </span>
