...
import { MdSettings, MdSecurity, MdBlock, MdCheckCircle, MdModeEdit } from "react-icons/md";
                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-slate-500 font-medium mt-1">Manage admin roles and account statuses</p>
                        </div>
                    </div>
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {admins.map((admin) => (
                                            <tr key={admin._id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className={`px-3 py-1 rounded-full text-xs font-bold flex w-fit items-center gap-1 uppercase ${admin.status === 'active'
                                                            ? 'bg-green-100 text-green-700'
                                                            : admin.status === 'inactive'
                                                                ? 'bg-yellow-100 text-yellow-700'
                                                                : 'bg-red-100 text-red-700'
                                                        }`}>
                                                        {admin.status === 'active' ? <MdCheckCircle /> : <MdBlock />}
                                                        {admin.status}
                                                    </span>
                                                </td>
                    ...
                    <div className="mt-8 bg-amber-50 rounded-xl p-4 border border-amber-100 flex gap-4 items-start">
                        ...
                        <div>
                            <p className="text-amber-800 font-bold">Security Notice</p>
                            <p className="text-amber-700 text-sm mt-0.5">
                                Changing another admin's status requires a super secret security key. This action is logged and monitored for system integrity.
                            </p>
                        </div>
                    </div>
                </div>
